        ...
        <div className="flex flex-wrap gap-2 items-center">
          {PRESETS.map(p => (
            <button key={p.label} onClick={() => setPreset(p)}
              className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${activePreset === p.label ? 'bg-[var(--primary)] text-white' : 'bg-[var(--surface-soft)] text-[var(--muted)] hover:text-[var(--text)]'}`}>
              {p.label}
            </button>
          ))}
          <div className="ml-auto flex items-center gap-1">
            {['day', 'week', 'month'].map(g => (
              <button key={g} onClick={() => setGroupBy(g)}
                className={`rounded-full px-3 py-1.5 text-xs font-semibold ${groupBy === g ? 'bg-[var(--primary)] text-white' : 'bg-[var(--surface-soft)] text-[var(--muted)]'}`}>
                {g}
              </button>
            ))}
          </div>
          ...
          <div className="surface-card rounded-[1.75rem] p-6 mb-6">
            <div className="mb-5 flex items-center justify-between">
              ...
              <button onClick={() => setCumulative(c => !c)}
                className={`rounded-full px-3 py-1.5 text-xs font-semibold ${cumulative ? 'bg-[var(--tertiary)] text-white' : 'bg-[var(--surface-soft)] text-[var(--muted)]'}`}>
